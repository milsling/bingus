...
export function DesktopLayout({ children, showSidebar = true }: DesktopLayoutProps) {
  ...
  return (
    <div className="hidden md:block min-h-screen bg-background relative overflow-hidden">
      {/* Ambient background effects */}
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-primary/8 rounded-full blur-[150px]" />
        <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-primary/6 rounded-full blur-[120px]" />
      ...
      <nav className="fixed top-4 left-4 right-4 h-14 z-50">
        <div className="h-full bg-foreground/[0.06] backdrop-blur-3xl border border-foreground/[0.05] rounded-full shadow-2xl shadow-black/30 flex items-center px-4">
          {/* Left Section - Hamburger + Logo */}
          <div className="flex items-center gap-1 min-w-[200px]">
            {/* Hamburger Menu Button */}
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-2.5 rounded-full hover:bg-foreground/[0.08] transition-colors"
      ...
      <div className="pt-24 min-h-screen">
        ...
        {showSidebar && (
          <aside className={cn(
            ...
            {currentUser ? (
              <div className="bg-foreground/[0.06] backdrop-blur-xl border border-foreground/[0.1] rounded-2xl p-4">
                {/* Profile Header */}
                <Link href="/profile">
                  <div className="flex flex-col items-center text-center cursor-pointer group">
                    <div className="relative mb-3">
                      <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary/30 to-purple-500/30 flex items-center justify-center overflow-hidden border-2 border-foreground/[0.15] group-hover:border-primary/50 transition-colors">
                    ...
                    <p className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors">@{currentUser.username}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">Level {currentUser.level || 1} · {currentUser.xp || 0} XP</p>
                  </div>
                </Link>

                {/* Stats Row */}
                <div className="flex justify-center gap-6 mt-4 pt-4 border-t border-foreground/5">
                  <div className="text-center">
                    <p className="text-lg font-bold text-foreground">{currentUser.xp || 0}</p>
                    <p className="text-[10px] text-muted-foreground uppercase">XP</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-bold text-foreground">{currentUser.level || 1}</p>
              ...
              <div className="bg-foreground/[0.06] backdrop-blur-xl border border-foreground/[0.1] rounded-2xl p-4 text-center">
                <User className="h-10 w-10 text-foreground/30 mx-auto mb-3" />
            ...
            <div className="flex-1 bg-foreground/[0.06] backdrop-blur-xl border border-foreground/[0.1] rounded-2xl p-4 flex flex-col overflow-y-auto">
              {/* Navigation Items */}
                ...
                <div className="flex flex-wrap gap-2">
                  {['Punchlines', 'Wordplay', 'Storytelling', 'Battle', 'Metaphor', 'Flow'].map((cat) => (
                    <Link key={cat} href={`/?category=${cat.toLowerCase()}`}>
                      <span className="inline-flex items-center px-3 py-1.5 rounded-full bg-foreground/[0.05] hover:bg-foreground/[0.1] border border-foreground/[0.08] text-sm text-foreground/70 hover:text-foreground transition-colors cursor-pointer">
                        {cat}
                      </span>
                    </Link>
                  ))}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {trendingHashtags.slice(0, 6).map((tag) => (
                      <Link key={tag.tag} href={`/?tag=${tag.tag}`}>
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-foreground/[0.05] hover:bg-foreground/[0.1] border border-foreground/[0.08] text-xs text-foreground/70 hover:text-foreground transition-colors cursor-pointer">
                          <Hash className="h-3 w-3 text-primary/70" />
                          {tag.tag}
                          <span className="text-foreground/30 ml-1">{tag.count}</span>
                        </span>
                      </Link>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2">
                    {onlineFriends.slice(0, 5).map((friend) => (
                      <Link key={friend.id} href={`/profile/${friend.username}`}>
                        <div className="flex items-center gap-2 p-2 rounded-lg hover:bg-foreground/[0.05] transition-colors cursor-pointer group">
                          <div className="relative">
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary/30 to-purple-500/30 flex items-center justify-center overflow-hidden border border-foreground/10">
                              {friend.avatarUrl ? (
                          ...
                          <span className="text-sm text-foreground/70 group-hover:text-foreground truncate">@{friend.username}</span>
                        </div>
                      </Link>
                    ))}
                  </div>
            ...
            <div className="h-full bg-foreground/[0.06] backdrop-blur-xl border border-foreground/[0.1] rounded-2xl p-4 flex flex-col gap-4 overflow-y-auto">
              ...
              {dailyChallenge && (
                <div className="p-4 bg-gradient-to-br from-amber-500/10 to-orange-500/10 rounded-xl border border-amber-500/20">
                  ...
                  <p className="text-sm text-foreground/90 mb-2">{dailyChallenge.prompt}</p>
              ...
              {/* Your Stats */}
              {currentUser && (
                <div className="p-4 bg-gradient-to-br from-primary/10 to-purple-500/10 rounded-xl border border-foreground/5">
                  <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Your Stats</h3>
                  ...
                  <div className="space-y-2">
                    {trendingBars.slice(0, 3).map((bar, idx) => (
                      <Link key={bar.id} href={`/bar/${bar.id}`}>
                        <div className="p-3 rounded-lg bg-foreground/[0.03] hover:bg-foreground/[0.06] border border-foreground/5 transition-colors cursor-pointer group">
                          <div className="flex items-start gap-2">
                            <span className="text-lg font-bold text-foreground/20">#{idx + 1}</span>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm text-foreground/80 group-hover:text-foreground line-clamp-2 leading-relaxed">
                                {bar.content.length > 60 ? bar.content.substring(0, 60) + '...' : bar.content}
                              </p>
                              <p className="text-xs text-muted-foreground mt-1">@{bar.user.username}</p>
                            </div>
                          </div>
                        </div>
                      </Link>
                  ...
                  <div className="space-y-2">
                    {leaderboard.slice(0, 5).map((user, idx) => (
                      <Link key={user.id} href={`/profile/${user.username}`}>
                        <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-foreground/[0.05] transition-colors cursor-pointer group">
                          <span className={cn(
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-foreground/80 group-hover:text-foreground truncate">@{user.username}</p>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {suggestedUsers.slice(0, 4).map((user) => (
                      <Link key={user.id} href={`/profile/${user.username}`}>
                        <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-foreground/[0.05] transition-colors cursor-pointer group">
                          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary/30 to-purple-500/30 flex items-center justify-center overflow-hidden border border-foreground/10">
                            {user.avatarUrl ? (
                              <img src={user.avatarUrl} alt="" className="w-full h-full object-cover" />
                            ) : (
                              <User className="h-4 w-4 text-foreground/50" />
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-foreground/80 group-hover:text-foreground truncate">@{user.username}</p>
                            <p className="text-xs text-muted-foreground">{user.barsCount} bars</p>
                          </div>
                        </div>
                      </Link>
                    ))}
                ...
                <div className="space-y-2">
                  <Link href="/?filter=top">
                    <div className="px-3 py-2.5 rounded-lg bg-gradient-to-r from-purple-500/10 to-pink-500/10 hover:from-purple-500/20 hover:to-pink-500/20 border border-purple-500/20 transition-colors cursor-pointer flex items-center gap-3">
                      ...
                      <span className="text-sm text-foreground/80">Leaderboard</span>
                    </div>
                  </Link>
                </div>
              </div>
              ...
              <div className="mt-auto pt-4 border-t border-foreground/5">
                ...
                <p className="text-xs text-muted-foreground mt-2">© 2025 Orphan Bars</p>
              </div>
            </div>
          </aside>
        )}
  ...
  return (
    <Link href={href}>
      <div className={cn(
        ...
        active 
          ? "bg-foreground/[0.08] text-foreground shadow-lg shadow-primary/10" 
        ...
        <Icon className={cn("h-5 w-5 transition-colors", active ? "text-primary" : "group-hover:text-foreground/70")} />
    ...
    <Link href={href}>
      <div className={cn(
        ...
        primary 
          ...
          : "bg-foreground/[0.03] border border-foreground/5 hover:bg-foreground/[0.06] hover:border-foreground/10"
      )}>
        <Icon className={cn("h-5 w-5 mx-auto mb-1.5 transition-transform group-hover:scale-110", primary ? "text-primary" : "text-foreground/50 group-hover:text-foreground/70")} />
        <span className={cn("text-[11px] font-medium", primary ? "text-foreground/90" : "text-foreground/50 group-hover:text-foreground/70")}>{label}</span>
      </div>
    </Link>
  );
}
  ...
  return (
    <Link href={href}>
      <div className={cn(
        ...
        active 
          ? "bg-foreground/[0.12] text-foreground shadow-lg" 
